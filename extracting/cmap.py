...
def main():
    ...
    for row in raw_data["features"]:  # all data is in the key "features"
        ...
        for head in headers:
            if head in age_headers:
                local_data[head] = round(row_dict[head] / total_pop * 100, 1)
            elif head in racial_headers:
                local_data[head] = round(row_dict[head] / total_race * 100, 1)
            elif head == "MED_RENT":
    ...
    csv_format = []
    for com in comm_id_features.values():
        csv_format.append(com)
